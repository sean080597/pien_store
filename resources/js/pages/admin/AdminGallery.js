...
export default function AdminGallery(props) {
  ...
  // hooks
  useInitializePageAdmin('gallery')
  const INITIAL = { title: '', description: '', widthRatio: 0, heightRatio: 0, keyword: '', input_image: '', origin_image: '' }
  const formFields = ['title', 'description', 'widthRatio', 'heightRatio', 'keyword', 'input_image']
  const { userInputs, errors, handleChange, handleBlur, handlePaginate, modalTitle, isEditing, isDeleting, isSubmitDisabled,
    ...
    handleSelectedFile } = useAdminActions(INITIAL, formFields, modalRef, 'gallery')
  // data
  ...
  const columns = useMemo(() => [
    ...
    { Header: 'Image', Cell: props => console.log(props.cell.row) },
    {
      Header: 'Ratio', columns: [
